class GovukXInclude extends HTMLElement {
	...
	parseVars() {
		...
		try {
			...
			console.warn("[govuk-x-include] Invalid JSON in vars:", raw);
			return {};
		}
	}
		...
		this.querySelectorAll("[data-active]").forEach((container) => {
			...
			container.querySelectorAll("[data-nav]").forEach((link) => {
				...
				if (isActive) {
					...
					if (!link.querySelector(".govuk-service-navigation__active-fallback")) {
						link.innerHTML = `<strong class="govuk-service-navigation__active-fallback">${link.innerHTML}</strong>`;
					}
					return;
				}
	...
	async render() {
		...
		try {
			...
			console.error("[govuk-x-include] render error:", source, error);
...
if (!customElements.get("x-include")) {
	customElements.define("x-include", GovukXInclude);
}

export function whenGovukIncludesReady(root = document) {
