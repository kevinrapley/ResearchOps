...
(() => {
		...
		while (Date.now() - start < maxMs) {
			try {
				const js = await fetch(
					`${API_ORIGIN}/api/mural/await?muralId=${encodeURIComponent(muralId)}&projectId=${encodeURIComponent(projectId)}&uid=${encodeURIComponent(uid())}`, { method: "GET", cache: "no-store" }
				).then(async r => {
	...
	// NEW: project name → Airtable record lookup so we can pass a real linked record id to /api/mural/setup
	async function resolveProjectIdByName(name) {
		const r = await fetch(
			`${API_ORIGIN}/api/projects/lookup-by-name?name=${encodeURIComponent(name)}`, { cache: "no-store" }
		);
		if (!r.ok) return null;
	...
	function setSetupAsCreate(projectId, projectName) {
		...
		els.btnSetup.onclick = async () => {
			try {
				...
				// NEW: look up the Airtable record id by project name so we can send a real linked record
				const nameForLookup = projectName || getProjectName() || "Project";
				const projectRecordId = await resolveProjectIdByName(nameForLookup);

				const body = {
					uid: uid(),
					projectName: nameForLookup,
				...
				if (boardUrl) {
					// keep cache keyed by the current page’s project id (slug or id) so UI “Open” works
