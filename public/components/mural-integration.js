 ...
 * - ?projectName=… (URL)
 *
 * Debug: logs go to console.* (captured by your page debug console).
 */
...
async function handleSetupClick(statusEl, btn, uid) {
  // Re-resolve project name at click time
  ...
  if (!projectName) {
    // Prompt as a last resort so the button always “does something”.
...
function init() {
  ...
  // Verify, then enable setup (we’ll prompt for a missing project name at click time)
  verify(uid).then((res) => {
    console.log("[mural] verify result:", res);
    const setupBtn = /** @type {HTMLButtonElement|null} */ ($("#mural-setup"));
    if (res.ok) {
      setPill(statusEl, "ok", "Connected to Mural (Home Office)");
      if (setupBtn) setupBtn.disabled = false; // ← always allow click; prompt if missing name
      const connectBtn = /** @type {HTMLButtonElement|null} */ ($("#mural-connect"));
  ...
  // If your dashboard injects the buttons later, keep listeners in sync:
