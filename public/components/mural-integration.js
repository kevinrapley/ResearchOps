...
function extractMuralOpenUrl(res) {
  // The Worker returns the Mural object under `res.mural.value`
  const v = res?.mural?.value || res?.mural || {};
  // Prefer a direct viewing link; fall back to canvas link or share link
  return (
  ...
  if (setupBtn && !setupBtn.__muralBound) {
    ...
    setupBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        // Always re-enable the button (it may immediately be changed to “Open …” above)
        setupBtn.disabled = false;
