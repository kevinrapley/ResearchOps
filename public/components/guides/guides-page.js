...
async function hydrateCrumbs({ pid, sid }) {
	try {
		...
		const studyRes = await fetch(`/api/studies/${encodeURIComponent(sid)}`, { cache: 'no-store' });
		const project = projRes.ok ? await projRes.json() : {};
		const study = studyRes.ok ? await studyRes.json() : {};

		$('#breadcrumb-project').href = `/pages/project/?pid=${encodeURIComponent(pid)}`;
		...
		$('#breadcrumb-study').textContent = study?.title || 'Study';
		$('[data-bind="study.title"]').textContent = study?.title || '—';

		// Stash for editor context
	...
	try {
		...
		for (const g of list) {
			...
			tr.innerHTML = `
				...
				<td>
					<button class="link-like" data-open="${g.id}">Open</button>
				</td>
			`;
...
function populateVariablesForm(meta) {
	...
	form.innerHTML = '';
	const fields = Object.entries(meta || {}).slice(0, 40); // simple cap
