...
Inferring the column geometry from board widgets was the source of the drift. Placement is now deterministic against the template grid: `placementForRow(layout, rowIndex)` places each card at the fixed column x (`COLUMN_X`), fixed width (288), fixed height (168), and y = `COLUMN_START_Y` (264) + `rowIndex` × `ROW_PITCH` (192). The row index is the entry's position within its category in the sorted hydrate list, so card placement does not depend on detecting "the previous card" — an earlier attempt that chained each card off the last-found card piled every card at the start row when the just-placed card was not re-matched. Matching (which card already holds an entry) still uses the tolerant derived layout and category/body matching, so existing cards are recognised and not duplicated; only placement is pinned to the grid. Runtime tests assert new cards land at x 120, width 288, and the second card at y 456 (264 + 192).

This row-index placement is the version validated working by the board owner; it was adopted wholesale for the layout module while retaining the tag work (Snowberry project-tag preservation, D1 project-name resolution, and the Mint user-tag creation/restyle in `mural-journal-sync-safe-tags.js`).
