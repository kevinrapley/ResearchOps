...
- Corrected the live failure where synced cards lost the Snowberry project tag: project-name resolution no longer depends on the browser payload. When project-route hydration fails in the browser (for example the preview 401 on `/api/projects/:id`), the client could only echo the project id or an unrelated page heading, so the project-name tag filter dropped the Snowberry tag from new cards. The sync now resolves the project name server-side from the D1 `projects` and `rops_projects_cache` records (checking all known Test Project 1 id aliases) and uses the client-sent name only as a fallback. Project names longer than Mural's 25-character tag limit match their truncated board tag.
- Corrected widget tag-application removal: because `PATCH` tag application replaces a widget's whole tag set, applying tags to an existing widget now merges the tags already on that widget (resolved by id or text, including widget tag objects whose ids are missing from the board tag list) so the Raspberry category and Snowberry project tags can no longer be stripped from a patched template sticky.
- Corrected user-created tags that were not Mint: tags created by earlier sync runs (or by hand) with default styling are now restyled to the Mint contract via the documented `PATCH /murals/{muralId}/tags/{tagId}` endpoint when they are user-authored ResearchOps tags. Board-curated tags (category and project tags) are never restyled.
- Aligned the compact journal Mural sync payload with the journal tabs payload: both now prefer the hydrated `main[data-project-name]` value before falling back to the page heading or project id.
...
The safe-tags wrapper applies existing Mural tags by id when the Mural tags list returns them. If a template carries a tag name that Mural does not report as an existing board tag, the wrapper will avoid creating that template tag as Mint; new cards will omit that missing template tag rather than mis-colouring it. Existing widgets keep such tags because tag application merges the tags already on the widget.

The Mint tag style uses the repository's Mint constants (`#DDF7E8FF` background, `#98DDB8FF` border, `#0B0C0CFF` text). Mural does not publicly document its built-in tag preset values, so if these differ from the workspace's built-in Mint preset the constants in `mural-journal-sync-safe-tags.js` are the single place to adjust.
