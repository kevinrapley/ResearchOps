...
- Branch: `fix/role-assignment-session-origin`
...
The code change was committed through the Git object workflow rather than a full-file `update_file` replacement:

1. created a corrected blob for `public/js/auth-role-assignment-page.js`
2. created a tree containing that blob
3. created a commit with the current branch head as parent
4. moved the branch ref with `force=false`

## Validation

Validation has not been executed locally in this ChatGPT environment. CI should run after the PR is opened.
