...
- Added a checked-in copy of the supplied `Research_Ethics_approval_form_v3.docx` template as an application asset under `public/templates/ethics/`.
...
- CI remediation: removed the application Word template from the governed Sourcebook template inventory path after the Sourcebook quality gate reported it as an unregistered Sourcebook template.
- CI remediation: updated the D1 migration ordering policy so the next main migration prefix follows `0026_ethics_submission_documents.sql`.
...
- `npm test -- tests/sourcebook-link-inventory.test.js` passed after moving the ethics submission template out of the governed Sourcebook template area.
- `node tests/d1-migration-ordering-route-state.test.js` passed.
- `npm test` passed with 356 tests.
