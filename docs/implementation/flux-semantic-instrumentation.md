...
The analyser is loaded before the tracker on every rendered page. Its dictionary is split into sub-512 KB static assets, and a failed or unavailable analyser degrades to the existing content-free timing event. Every transport re-checks the current consent state, so a pending asynchronous dictionary result is discarded if consent has been revoked. The consent banner tells visitors that possible UK English issues are measured locally and that their text does not leave the page.
