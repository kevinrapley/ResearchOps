...
Before capture, the tracker writes semantic attributes onto every current interactive element and observes newly rendered controls. Explicit publisher keys always win. Missing keys are derived only from controlled structural attributes such as an element ID, field name, allow-listed action or option value, safe HTTP(S) destination path, controlled page key, region or form identity. `mailto:` and `tel:` destinations become fixed contact-purpose keys; other non-page protocols become a fixed external-action key. Query strings, visible text, accessible names, entered values, arbitrary data-attribute values and framework-added initialization attributes are never used. Checkbox and radio options prefer an allow-listed controlled value, otherwise a sanitised option ID with only short structural ordinals retained. A positional `auto.*` key remains the final defensive fallback for truly contextless controls rather than assigning a misleading generic semantic key. Normal ResearchOps controls should receive a purpose-led key before an interaction occurs. Focus-management and display-only targets are excluded from control capture; custom controls must expose an interactive role such as `role="button"`.
...
The analyser is loaded before the tracker on every rendered page. Its dictionary is split into sub-512 KB static assets and fetched with same-origin credentials because ResearchOps protects static dependencies with the authenticated service boundary; the cross-origin collector continues to omit credentials. A failed or unavailable analyser degrades to the existing content-free timing event. Every transport re-checks the current consent state, so a pending asynchronous dictionary result is discarded if consent has been revoked. The consent banner tells visitors that possible UK English issues are measured locally and that their text does not leave the page.
