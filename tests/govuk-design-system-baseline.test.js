...
const complianceAudit = fs.readFileSync(
  "docs/design-system/govuk-compliance-audit.md",
  "utf8",
);
const componentInventory = fs.readFileSync(
...
includes(
  complianceAudit,
  "Route-level stylesheets are only for page-specific or deliberately divergent styles",
  "GOV.UK compliance audit",
);
...
includes(
  componentInventory,
  "Do not move or duplicate a shared component into route CSS",
...
includes(
  performanceAudit,
  "Design-system work must preserve the CSS ownership doctrine",
  "performance audit",
);
includes(
  performanceAudit,
