...
includes("`${API_ORIGIN}/api/mural/auth?return=${encodeURIComponent(backPath)}`");
includes("`${API_ORIGIN}/api/mural/verify`");
includes("`${API_ORIGIN}/api/mural/setup`");
includes("`${API_ORIGIN}/api/mural/resolve?projectId=${encodeURIComponent(pid)}`");
...
excludes("function uid()");
excludes("mural.uid");
excludes("uid: uid()");

configIncludes('PAGES_ORIGIN       = "https://researchops.pages.dev"');
configIncludes("https://researchops.pages.dev");
