...
includes(workerSource, "route_api_card_sorts_config_get", "Worker route permissions");
includes(workerSource, "route_api_card_sorts_config_post", "Worker route permissions");
includes(workerSource, "route_api_card_sorts_results_get", "Worker route permissions");
includes(workerSource, "route_api_card_sorts_results_post", "Worker route permissions");
includes(workerSource, "route_api_card_sorts_result_patch", "Worker route permissions");
includes(workerSource, 'requestForRoutePermission(request, "/api/card-sorts/results/:id")', "Worker route permissions");
...
includes(setupScript, 'apiUrl("/api/studies")', "card sort setup script");
excludes(setupScript, "/api/studies/${encodeURIComponent", "card sort setup script");
...
includes(sessionController, 'apiUrl("/api/studies")', "session card sort controller");
excludes(sessionController, "/api/studies/${encodeURIComponent", "session card sort controller");
includes(sessionShellController, 'apiUrl("/api/studies")', "session controller");
excludes(sessionShellController, "/api/studies/${encodeURIComponent", "session controller");
includes(sessionController, "saveGeneration", "session card sort controller");
includes(sessionController, "flushPendingSaveBeforeParticipantSwitch", "session card sort controller");
includes(sessionController, "participantId === state.participantId", "session card sort controller");
includes(sessionController, "canCompleteCardSort", "session card sort controller");
includes(sessionController, "state.unsorted.length === 0", "session card sort controller");
...
includes(sessionController, "Sort every card into a group before marking the card sort complete.", "session card sort controller");
