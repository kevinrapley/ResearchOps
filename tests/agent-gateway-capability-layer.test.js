...
const files = {
  ...
  allowlist: fs.readFileSync(
    `${gatewayRoot}/src/policies/allowlist.ts`,
    "utf8",
  ),
...
includes(
  files.allowlist,
  "approved production-safe prefix",
  "agent gateway allowlist",
);
includes(
  files.allowlist,
...
includes(
  files.audit,
  "AUDIT_DB binding is required",
  "agent gateway audit log",
);
...
includes(
  files.readme,
  "production-safe capability layer",
