...
assert.match(head, /src="\/js\/flux-researchops-tracker\.1\.2\.0\.js"/);
assert.match(renderedLayout, /src="\/js\/flux-researchops-tracker\.1\.2\.0\.js"/);
...
for (const relativePath of renderedPages) {
	...
	assert.match(fs.readFileSync(page, 'utf8'), /src="\/js\/flux-researchops-tracker\.1\.2\.0\.js"/, `${page} should load the Flux tracker`);
}
...
assert.match(sessionTracker, /flux-researchops-tracker\.1\.1\.1\.js/);
