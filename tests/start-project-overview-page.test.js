...
const visualWalkthroughConfig = fs.readFileSync(
...
assert.equal(
  headingText(overviewPage, 1).length,
  1,
  "Start overview page should have one h1",
);
...
assert.equal(
  overviewPage.includes('class="govuk-button" href="/pages/start/"'),
  true,
);
assert.equal(
  textContent(
    overviewPage,
    /<a\b[^>]*class="govuk-button"[^>]*>([\s\S]*?)<\/a>/,
  ),
  "Start now",
);
assert.equal(
  overviewPage.includes('<h2 class="govuk-heading-m">Before you start</h2>'),
  true,
);
assert.equal(
  overviewPage.includes('<h2 class="govuk-heading-m">What you will do</h2>'),
  true,
);
assert.equal(
  overviewPage.includes(
...
assert.equal(
  overviewPage.includes('class="govuk-list govuk-list--bullet"'),
  true,
);
assert.equal(
  overviewPage.includes('class="govuk-list govuk-list--number"'),
  true,
);
