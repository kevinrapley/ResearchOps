...
const REPORTS_DIR = path.join(ROOT_DIR, "reports-site");
...
function readJson(filePath) {
  try {
    ...
    fail(`invalid JSON in ${path.relative(ROOT_DIR, filePath)}: ${error.message}`);
  }
}
...
function validateReportsSite({ rootDir = ROOT_DIR } = {}) {
  ...
  const screenshotFiles = fs
    .readdirSync(path.join(reportsDir, "screenshots"), { recursive: true, withFileTypes: true })
    .filter((entry) => entry.isFile())
    .map((entry) => path.join(entry.parentPath, entry.name))
    .map((filePath) => path.relative(reportsDir, filePath).replaceAll(path.sep, "/"));
