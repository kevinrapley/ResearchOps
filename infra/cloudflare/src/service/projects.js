 ...
 * @summary Project-related handlers with robust GitHub CSV fallback.
 */
...
/** Super-light CSV parser (no support for embedded commas/newlines). */
...
/** Common JSON headers. */
function jsonHeaders(ctx, origin) {
	return {
 ...
 * Always returns: { ok:true, projects:[…] }
 * Falls back to GitHub CSV if Airtable fails or returns non-JSON (e.g. SPA HTML).
...
export async function listProjectsFromAirtable(ctx, origin, url) {
	...
	const attemptAirtable = async () => {
		...
		const pType = (pRes.headers.get("content-type") || "").toLowerCase();
		const pText = await pRes.text();

		if (!pType.includes("application/json")) {
			ctx.log.warn("airtable.projects.nonjson", { status: pRes.status, preview: safeText(pText).slice(0, 160) });
		...
		if (dRes.ok && (dRes.headers.get("content-type") || "").toLowerCase().includes("application/json")) {
			...
			ctx.log.warn("airtable.details.join.fail", { status: dRes.status, detail: safeText(dt).slice(0, 160) });
		}
	...
	/* Try Airtable first; if anything goes wrong, fall back to GitHub CSV. */
	let projects = null;
	try {
		projects = await attemptAirtable();
	} catch (airErr) {
		...
		try {
			...
			projects = rows.map(coerceCsvRowToProject);
	...
	// Always return object shape for the dashboard
	return ctx.json({ ok: true, projects }, 200, jsonHeaders(ctx, origin));
}
...
export async function getProjectById(ctx, origin, projectId) {
	...
	return ctx.json(project, 200, jsonHeaders(ctx, origin));
}
