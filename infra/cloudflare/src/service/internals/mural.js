...
export class MuralServicePart {
	 ...
	 * Priority: 
	 *   1. Explicit body.muralId (from client localStorage)
	 *   2. KV mapping (from setup endpoint)
	 *   3. env.MURAL_REFLEXIVE_MURAL_ID (deprecated global fallback)
	 */
	async resolveReflexiveMuralId(projectId, explicitMuralId) {
		// 1. Explicit from client (highest priority)
		if (explicitMuralId) {
			console.log('[mural] using explicit muralId:', explicitMuralId);
			return explicitMuralId;
		}

		// 2. KV mapping (per-project)
		if (projectId) {
			const raw = await this.root.env.SESSION_KV.get(this.projectMuralKey(projectId));
			if (raw) {
				try {
					const js = JSON.parse(raw);
					const id = js?.muralId || js?.id || null;
					if (id) {
						console.log('[mural] resolved from KV for project:', projectId, '→', id);
						return id;
					}
				} catch {
					console.log('[mural] resolved from KV (plain string):', raw);
					return raw;
				}
			}
		}

		// 3. Global fallback (deprecated - logs warning)
		const envId = this.root?.env?.MURAL_REFLEXIVE_MURAL_ID;
		if (envId) {
			console.warn('[mural] using deprecated global MURAL_REFLEXIVE_MURAL_ID — migrate to per-project mappings');
			return String(envId);
		}

		console.error('[mural] no muralId found for project:', projectId);
		return null;
	}
