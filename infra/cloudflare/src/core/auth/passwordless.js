...
async function roles(db, userId) {
	const result = await db
		.prepare(`
		...
		WHERE ra.user_id = ? AND ra.assignment_status = 'active'
...
async function permissions(db, userId) {
	const result = await db
		.prepare(`
...
export async function resolvePasswordlessSessionContext(request, env) {
	...
	const pending = (async () => {
		...
		const userRoles = await roles(db, user.id);
		const userPermissions = await permissions(db, user.id);
		return {
			authenticated: true,
			provider: PROVIDER,
