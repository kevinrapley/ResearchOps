...
function dbFor(env = {}) {
	...
	if (!db || typeof db.prepare !== 'function') {
		throw new RoleAssignmentError(503, 'role_assignment_store_unavailable', 'Role assignments cannot be changed right now.');
	}
	return db;
}
...
function requestedTeamIdFor(body, context) {
	...
	if (!teamId) throw new RoleAssignmentError(403, 'active_team_required', 'Choose an active team before assigning roles.');
	return teamId;
}

function requestedNewTeamNameFor(body) {
	const name = cleanSingleLine(body.newTeamName);
	if (name.length < 3) throw new RoleAssignmentError(400, 'new_team_name_required', 'Provide a team name of at least 3 characters.');
	if (name.length > 80) throw new RoleAssignmentError(400, 'new_team_name_too_long', 'Team names must be 80 characters or fewer.');
	return name;
}

function requestedRoleKeyFor(body) {
	const roleKey = cleanText(body.roleKey);
	if (!roleKey) throw new RoleAssignmentError(400, 'role_required', 'Provide roleKey for the role to assign.');
	return roleKey;
}

function requestedReasonFor(body) {
	const reason = cleanText(body.requestedReason);
	if (reason.length < 12) throw new RoleAssignmentError(400, 'role_assignment_reason_required', 'Provide a clear reason for assigning this role.');
	return reason;
}
...
function targetSelectorFor(body) {
	...
	if (!targetUserId && !targetEmail) {
		throw new RoleAssignmentError(400, 'target_required', 'Provide targetUserId or targetEmail for the user receiving the role.');
	}
...
function expiresAtFor(body) {
	...
	if (Number.isNaN(parsed)) throw new RoleAssignmentError(400, 'invalid_expiry', 'expiresAt must be an ISO-8601 date-time when provided.');
...
function manageableTeamsFor(context) {
	return context.manageableTeams || context.teams || [];
}
...
async function canAssignRolesInTeam(db, context, teamId) {
	if (context.isResearchOpsCoreTeamAdmin) return true;
	const row = await db
		.prepare(`
			SELECT p.code
...
async function readTargetUser(db, targetUserId, targetEmail) {
	...
	if (targetUserId && targetEmail) {
		if (!userById || !userByEmail || userById.id !== userByEmail.id) {
			throw new RoleAssignmentError(400, 'target_identifier_conflict', 'targetUserId and targetEmail must resolve to the same user.');
		}
		return userById;
	}
...
function assertSensitiveRoleConfirmation(role, body) {
	const sensitive = role.is_sensitive === 1 || role.approval_required === 1;
	if (!sensitive) return;
	if (body.sensitiveRoleConfirmation !== 'ASSIGN_SENSITIVE_ROLE') {
		throw new RoleAssignmentError(400, 'sensitive_role_confirmation_required', 'Confirm that this sensitive role assignment is intentional.');
	}
	if (role.role_key === 'safeguarding_lead' && body.safeguardingConfirmation !== 'ASSIGN_SAFEGUARDING_LEAD') {
		throw new RoleAssignmentError(400, 'safeguarding_role_confirmation_required', 'Confirm that safeguarding lead access is intentionally required.');
	}
}

function prepareCreateTeamStatement(db, team) {
	return db.prepare("INSERT INTO auth_teams (id, name, team_status) VALUES (?, ?, 'active')").bind(team.id, team.name);
}

function prepareMembershipStatement(db, userId, teamId) {
	return db
		.prepare(`
		...
		`)
		.bind(stableMembershipId(userId, teamId), userId, teamId);
}

function prepareActivateUserStatement(db, targetUser) {
	return db
		.prepare("UPDATE auth_users SET account_status = 'active', updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = ? AND account_status = 'pending'")
		.bind(targetUser.id);
}
...
function prepareTeamCreationAuditStatement(db, request, context, team, reason) {
	...
	return db
		.prepare(`
			INSERT INTO auth_audit_events
				(id, event_type, actor_user_id, team_id, target_type, target_id, permission_code, route_path, outcome, is_safeguarding, metadata_json)
			VALUES (?, 'auth.team.created', ?, ?, 'auth_team', ?, 'team.manage', ?, 'succeeded', 0, json_object('team_name', ?, 'reason', ?))
		`)
...
async function buildNewAssignmentTeam(db, request, context, body) {
	...
	if (existingTeam) throw new RoleAssignmentError(409, 'team_name_already_exists', 'A team with this name already exists.');
	const teamAdminRole = await readTeamAdminRole(db);
	if (!teamAdminRole) throw new RoleAssignmentError(503, 'team_admin_role_unavailable', 'ResearchOps cannot create a team right now.');
	const team = { id: makeId('team'), name: teamName, created: true };
	const reason = cleanText(body.newTeamReason);
...
async function resolveExistingAssignmentTeam(db, context, body) {
	...
	const team = manageableTeamsFor(context).find((candidate) => candidate.id === teamId);
	if (!team) throw new RoleAssignmentError(403, 'team_not_available', 'You cannot assign roles in that team.');
	if (!(await canAssignRolesInTeam(db, context, team.id))) {
		throw new RoleAssignmentError(403, 'selected_team_role_assignment_forbidden', 'You do not have permission to assign roles in that team.');
	}
	return { ...team, created: false, preAssignmentStatements: [] };
}

async function resolveAssignmentTeam(db, request, context, body) {
	if (cleanText(body.teamAction) === CREATE_TEAM_ACTION) return buildNewAssignmentTeam(db, request, context, body);
...
async function assignRole(request, env, context, body) {
	...
	if (!role) throw new RoleAssignmentError(404, 'role_not_found', 'The requested role does not exist.');
	assertSensitiveRoleConfirmation(role, body);
	const targetUser = await readTargetUser(db, targetUserId, targetEmail);
	if (!targetUser) throw new RoleAssignmentError(404, 'target_user_not_found', 'The target user does not exist.');
	...
	const assignmentResult = await writeAssignmentWithAudit(db, request, context, targetUser, role, team, requestedReason, expiresAt, membership);
	return { assignmentResult, role, targetUser, team };
}
...
export async function handleRoleAssignmentsRoute(request, env) {
	try {
		...
		return jsonResponse(
			{
				...
				teamMembership: {
					status: 'active',
					createdOrReactivated: result.assignmentResult.membershipActivated,
				},
				role: {
				...
				team: { id: result.team.id, name: result.team.name, created: result.team.created === true },
				targetUser: {
					id: result.targetUser.id,
					email: result.targetUser.email,
					displayName: result.targetUser.display_name,
					accountStatus: result.assignmentResult.accountActivated ? 'active' : result.targetUser.account_status,
				},
			},
			201,
		);
		...
		if (error?.status && error?.code) return jsonResponse({ ok: false, error: error.code, message: error.message }, error.status);
		return jsonResponse({ ok: false, error: 'role_assignment_error', message: 'Role assignment could not be completed.' }, 500);
	}
}
