-- Preview-only data correction for account dashboard review
-- Date: 2026-05-13
-- Scope: move kevin.rapley@research-operations.com Observer access from
-- ResearchOps Core Team to DaaS in the preview D1 database.
--
-- This file is intentionally under migrations/preview and must not be applied
-- to production. It corrects review data created while testing PR #250.
...
SELECT
	...
	'active',
	'Correct preview data so this user is an Observer in DaaS.',
	(
		SELECT id
		FROM auth_users
...
SELECT
	'audit_preview_kevin_research_operations_daas_' || strftime('%Y%m%d%H%M%S', 'now'),
	'auth.preview_data.corrected',
	NULL,
	(
		SELECT id
		FROM auth_teams
	...
	u.id,
	'role.assign',
	'/preview/data-correction',
	NULL,
	'preview-account-access',
	'succeeded',
	0,
	'{"email":"kevin.rapley@research-operations.com","team":"DaaS","role":"Observer","reason":"Correct PR preview data after role assignment review."}',
	strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
FROM auth_users u
WHERE lower(u.email) = lower('kevin.rapley@research-operations.com')
	AND NOT EXISTS (
		...
		WHERE a.event_type = 'auth.preview_data.corrected'
			AND a.target_id = u.id
			AND a.field_group = 'preview-account-access'
